...
const main = async () => {
  ...
  async function replyMessage(auth, threadId) {
    ...
    const from = header.data.payload.headers.find((header) => header.name === "From").value;
    const to = header.data.payload.headers.find((header) => header.name === "To").value;
    console.log(from, to);
  ...
  const generateRawMessage = (to, from, message1) => {
    ...
    const messageParts = [
      `From: Satya Prakash <${to}>`,
      `To: ${from}`,
      "Content-Type: text/html; charset=utf-8",
      "MIME-Version: 1.0",
      `Subject: ${utf8Subject}`,
      "",
      "Dear Sender, Thank you for reaching out. Currently, I'm on parental leave and will contact you as soon as possible. Meanwhile, if it is urgent, you can call me at 9334223065.",
    ];
